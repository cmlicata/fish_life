...
### Insert data into the `temp_gallery_insert` table
  ...
  - Click `"Import"` and Volila! You now have all your (new) data in the `temp_insert_gallery` table!
...
### Update unpopulated `species_id` data in the `temp_gallery_insert` table
...
### Insert Updated Data in the `temp_insert_gallery` table
  ...
  - Validate that all of data was inserted into the `gallery` from the `temp_insert_gallery` table correctly.
  ...
  - Validate that all of data was deleted from the `temp_insert_gallery` table  correctly.
